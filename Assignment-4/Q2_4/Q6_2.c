...
int main()
{
  ...
  sigaction(SIGTERM, &action, NULL);
	...
	data data_to_receive = {0};
  data *	my_ptr = &data_to_receive;
	int ret = mq_receive(myqueue, my_ptr, sizeof(data_to_receive),0);
	if(ret == -1)
	{
		perror("error");
		exit(1);
  ...
  fclose(ft1);
    ...
    fprintf(ft1,"\n[Value]time stamp reading when parent sends to child: %ld \n", t.tv_nsec);
    fclose(ft1);
	ret = mq_send(myqueue, &data_to_sent, sizeof(data_to_sent),0);
	if(ret == -1)
	{
		perror("error");
		exit(1);
